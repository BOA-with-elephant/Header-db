...
CREATE TABLE IF NOT EXISTS `TBL_SHOP_MSG_HISTORY` (
    `SMH_CODE` INT PRIMARY KEY AUTO_INCREMENT,
    `USER_CODE` INT NOT NULL COMMENT '수신자',
    `SHOP_CODE` INT NOT NULL COMMENT '샵 코드',
    `DATE` DATE NOT NULL COMMENT '날짜',
    `TIME` TIME NOT NULL COMMENT '시간',
    `MSG_CONTENT` VARCHAR(255) NOT NULL COMMENT '메세지 내용',
    `MSG_TYPE` VARCHAR(20) NOT NULL COMMENT '메세지타입'
);
...
ALTER TABLE `TBL_SHOP_MSG_HISTORY` ADD CONSTRAINT `FK_TBL_SHOP_TO_SHOP_MSG_HISTORY_1` FOREIGN KEY (`SHOP_CODE`) REFERENCES `TBL_SHOP` (`SHOP_CODE`);